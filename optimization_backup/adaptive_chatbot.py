...
🤖 Bot: 👋 Chat band kar raha hun. Alvida!")
    ...
    def _get_chat_response(self, query: str) -> str:
        ...
        try:
            ...
            greetings = ['hello', 'hi', 'namaste', 'namaskar', 'helo']
            if any(word in query_lower for word in greetings):
                return "🙏 Namaste! Main aapki madad ke liye hun. Aap mujhse kuch bhi pooch sakte hain."
            
            how_are_you = ['how are you', 'kaise ho', 'kaise hain', 'kaisi ho']
            if any(phrase in query_lower for phrase in how_are_you):
                return "😊 Main theek hun, dhanyawad! Aap kaise hain?"
            
            thanks = ['thank you', 'thanks', 'dhanyawad', 'shukriya']
            if any(word in query_lower for word in thanks):
                return "🤗 Aapka swagat hai! Kuch aur poochna chahte hain?"
            
            name_queries = ['name', 'naam', 'who are you', 'aap kaun hain', 'tumhara naam']
            if any(word in query_lower for word in name_queries):
                app_name = config.get('app', 'app_name', 'Adaptive Chatbot')
                return f"🤖 Main {app_name} hun. Main seekhta rehta hun aur aapki madad karta hun."
...
Bas aap mujhse kuch bhi poochiye ya 'teach' kahkar mujhe nayi cheezon sikhaiye!"""
            ...
            # Default response
            return "🤔 Mujhe iska jawab nahi pata. Aap mujhe iske baare mein sikha sakte hain."
